...
int get_NEFHeader(Kneron_NEFContent_table_t* table_p, struct kdp_metadata_s *mdata) {
    if (table_p == NULL) {
        return -1;
    }
...
int get_nef_info(Kneron_NEFContent_table_t* table_p, struct kdp_nef_info_s *nef_info) {
...
int read_nef(char* nef_data, uint32_t nef_size, struct kdp_metadata_s *metadata, \
    ...
    int nef_crc = crc_cal((u8 *)nef_data, nef_size-4);
    int crc = *(u32 *)&(nef_data[nef_size-4]);
