...
int get_NEFHeader(Kneron_NEFContent_table_t* table_p, struct kdp2_metadata_s *mdata) {
    if (table_p == NULL) {
        return -1;
    }
    ...
    //printf("platform: %s\n", Kneron_NEFHeader_platform(nef_header));
    //printf("target: %d\n", Kneron_NEFHeader_target(nef_header));
    //printf("crc: 0x%08x\n", Kneron_NEFHeader_crc(nef_header));
    //printf("kn_number: 0x%08x\n", Kneron_NEFHeader_kn_number(nef_header));
    //printf("encryption_mode: %d\n", Kneron_NEFHeader_encryption_mode(nef_header));
    //printf("compiler v: %s\n", Kneron_NEFHeader_compiler_version(nef_header));
    //printf("toolchain v: %s\n", Kneron_NEFHeader_toolchain_version(nef_header));

    Kneron_SchemaVersion_table_t schema_v = Kneron_NEFHeader_schema_version(nef_header);

    if (schema_v == NULL) {
        return -1;
    }

    //printf("schema version major:%d\n", Kneron_SchemaVersion_major_num(schema_v));
    //printf("schema version minor:%d\n", Kneron_SchemaVersion_minor_num(schema_v));
    //printf("schema version revision:%d\n", Kneron_SchemaVersion_revision_num(schema_v));

    memcpy(mdata->platform, Kneron_NEFHeader_platform(nef_header), sizeof(mdata->platform));
    ...
    memcpy(mdata->tc_ver, Kneron_NEFHeader_toolchain_version(nef_header), sizeof(mdata->tc_ver));
    memcpy(mdata->compiler_ver, Kneron_NEFHeader_compiler_version(nef_header), sizeof(mdata->compiler_ver));
    return 0;
}
...
int get_nef_info(Kneron_NEFContent_table_t* table_p, struct kdp2_nef_info_s *nef_info) {
...
int read_nef(char* nef_data, uint32_t nef_size, struct kdp2_metadata_s *metadata, \
