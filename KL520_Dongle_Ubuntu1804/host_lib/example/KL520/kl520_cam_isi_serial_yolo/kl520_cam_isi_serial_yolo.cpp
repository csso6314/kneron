/**
 * @file      kl520_cam_isi_serial_yolo.cpp
...
/**
 * example for isi serial yolo with camera inputs
 */
int user_test_yolo(int dev_idx)
{
    int ret = 0;
    ...
    if (1) {
        ...
        while (!check_ctl_break()) {
            ...
            ret = kdp_isi_retrieve_res(dev_idx, img_id, &error_code, &result_size, inf_res);
            loop1++;
...
{
    //object detection test with tiny_yolo_v3
    user_test_yolo(dev_idx);
