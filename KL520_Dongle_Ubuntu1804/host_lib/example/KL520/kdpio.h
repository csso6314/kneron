...
#include <stdint.h>
/* header file in /common/include */
#include "ipc.h"
#define MAX_MODEL_REGISTRATIONS          32
#define MAX_OUTPUT_NUMBER                32
...
struct kdp_post_proc_s {
    ...
    struct out_node_s     node_p[MAX_OUTPUT_NUMBER];
...
#define POSTPROC_OUT_NODE(image_p, i)          (image_p->postproc.node_p[i])
#define POSTPROC_OUT_NODE_COL(image_p, i)      (image_p->postproc.node_p[i].col_length)
#define POSTPROC_OUT_NODE_ROW(image_p, i)      (image_p->postproc.node_p[i].row_length)
#define POSTPROC_OUT_NODE_CH(image_p, i)       (image_p->postproc.node_p[i].ch_length)
#define POSTPROC_OUT_NODE_RADIX(image_p, i)    (image_p->postproc.node_p[i].output_radix)
#define POSTPROC_OUT_NODE_SCALE(image_p, i)    (image_p->postproc.node_p[i].output_scale)
#define POSTPROC_OUT_NODE_ADDR(image_p, i)     (image_p->postproc.node_p[i].node_list[0].addr)
...
typedef int (* cpu_op_fn)(int cpu_op, struct kdp_image_s *image_p);
typedef int (* pre_proc_fn)(int model_id, struct kdp_image_s *image_p);
typedef int (* post_proc_fn)(int model_id, struct kdp_image_s *image_p);
...
/* Pre-/Post-/CPU-processing function provided by Kneron */
int kdp_preproc_fr(int model_id, struct kdp_image_s *image_p);
int kdp_preproc_fr_mask(int model_id, struct kdp_image_s *image_p);
int kdp_lv_preproc(int model_id, struct kdp_image_s *image_p);
int post_yolo(int model_id, struct kdp_image_s *image_p);
int post_ssd_face_detection(int model_id, struct kdp_image_s *image_p);
int post_imagenet_classification(int model_id, struct kdp_image_s *image_p);
int post_face_detection(int model_id, struct kdp_image_s *image_p);
int post_face_landmark_onet_5p(int model_id, struct kdp_image_s *image_p);
int post_face_recognition(int model_id, struct kdp_image_s *image_p);
int post_face_recognition_mask(int model_id, struct kdp_image_s *image_p);
int post_nir_liveness(int model_id, struct kdp_image_s *image_p);
